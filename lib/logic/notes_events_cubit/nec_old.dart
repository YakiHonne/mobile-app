...
//   final Set<String> alreadySearchedContentIds = <String>{};
//   final List<String> _notesIds = <String>[];
//   final List<String> _aTags = <String>[];
//   final Map<String, Event> _pendingNotesEvents = <String, Event>{};
...
//     final eventStats = await nc.db.loadEventStats(id);
...
//   Stream<EventStats?> watchEventStats(String id) {
//     return nc.db.watchEventStats(id);
//   }
...
//   Future<EventStats?> getCachedEventStats(String id) async {
//     return nc.db.loadEventStats(id);
//   }
...
//   }) async {
//     final eventStats = await getCachedEventStats(id);
//     if (eventStats == null) {
//       return [];
//     }
...
//     eventsByParent.forEach((id, events) async {
//       await processEvents(id, events);
//     });
//   }
...
//     final nStats = (await getCachedEventStats(id)) ??
//         EventStats(
//           eventId: id,
//           reactions: const {},
//           replies: const {},
...
//     final eventStats =
//         (await getCachedEventStats(note.id)) ?? EventStats.empty(note.id);
...
//     if (repostId.isEmpty) {
//       await _createRepost(note, eventStats);
//     } else {
//       await _deleteRepost(repostId, eventStats);
//     }
//   }
...
//     final eventStats = (await getCachedEventStats(id)) ?? EventStats.empty(id);
...
//     if (voteId.isEmpty) {
//       await _createReaction(id, pubkey, r, eventStats, customReaction);
//     } else {
...
//       if (shouldReplace) {
//         await _deleteReaction(voteId, eventStats);

//         final es = await getCachedEventStats(id) ?? EventStats.empty(id);
//         await _createReaction(id, pubkey, r, es, customReaction);
//       } else {
//         await _deleteReaction(voteId, eventStats);
//       }
//     }
...
//   Future<void> handleSubmittedZap({
...
//   }) async {
//     final eventStats =
//         (await nc.db.loadEventStats(eventId)) ?? EventStats.empty(eventId);
...
//   Future<void> replaceOptimisticZapWithReal({
//     required String eventId,
//     required String tempZapId,
//     required Event realEvent,
//   }) async {
//     final eventStats = await nc.db.loadEventStats(eventId);
...
//   Future<void> addEventRelatedData({
//     required Event event,
//     required String replyNoteId,
//   }) async {
//     final eventStats = (await nc.db.loadEventStats(replyNoteId)) ??
//         EventStats.empty(replyNoteId);
